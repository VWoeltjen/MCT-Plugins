 ...
 * @author Julien Ponge
 */
public class MCTFinishPanel extends IzPanel
{
     ...
     */
    public MCTFinishPanel(InstallerFrame parent, InstallData idata)
    {
    ...
    public void panelActivate()
    {
        ...
        if (idata.installSuccess)
        {
			...
			String path = translatePath(idata.getVariable("INSTALL_PATH"));
			...
			add(IzPanelLayout.createVerticalStrut(5), NEXT_LINE);
			add(IzPanelLayout.createVerticalStrut(5), NEXT_LINE);
			
			JPanel p = new JPanel();
			p.setLayout(new BoxLayout(p, BoxLayout.Y_AXIS));
			p.add(new JLabel("To start MCT Evaluation, navigate to:"));
			p.add(new JLabel(path));
			p.add(new JLabel("Then, run the file \"Launch MCT\""));
